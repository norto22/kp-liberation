...
private _grp = KPLIB_groups select (lbCurSel _ctrlGroupList);
...
if (leader _grp == player) then {
    ...
    // Refresh Dialog
    closeDialog 0;
    [{!dialog}, {call KPLIB_fnc_openDialog;}] call CBA_fnc_waitUntilAndExecute;
} else {
    hint localize "STR_KPLIB_NOTLEADER";
