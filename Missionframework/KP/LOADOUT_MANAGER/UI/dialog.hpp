...
class KPLIB_dialog {
    ...
    onLoad = "KPLIB_groups = allGroups select {isPlayer (leader _x)}";
    onUnload = "KPLIB_groups = nil";

    class controlsBackground {

        class KP_DialogTitle: KPLIB_GUI_PRE_DialogTitleC {
            text = "$STR_KPLIB_TITLE";
        };

        class KP_DialogArea: KPLIB_GUI_PRE_DialogBackgroundC {};

        class KP_LabelPlayer: KPLIB_GUI_PRE_InlineTitle {
            text = "$STR_KPLIB_LABELPLAYER";
        ...
        class KP_TextRank: KPLIB_GUI_PRE_Text {
            idc = 758031;
            text = "$STR_KPLIB_RANK";
            x = KP_GETCX(KP_X_VAL_C,KP_WIDTH_VAL_C,0,1);
        ...
        class KP_LabelView: KP_LabelPlayer {
            text = "$STR_KPLIB_LABELVIEW";
            y = KP_GETCY(KP_Y_VAL_C,KP_HEIGHT_VAL_C,1,2);
        };

        class KP_TextViewFoot: KPLIB_GUI_PRE_Text {
            text = "$STR_KPLIB_VIEWFOOT";
    ...
    class controls {
        ...
        class KPLIB_Version: KPLIB_GUI_PRE_DialogVersionIconC {
            tooltip = "$STR_KPLIB_VERSION";
        };

        class KP_PlayerRank: KPLIB_GUI_PRE_Text {
            idc = 758032;
            style = 2;
            text = "";
            x = KP_GETCX(KP_X_VAL_C,KP_WIDTH_VAL_C,1,2);
        ...
        class KP_GroupList: KPLIB_GUI_PRE_ListBox {
            idc = 758038;
            x = KP_GETCX(KP_X_VAL_C,KP_WIDTH_VAL_C,0,1);
            y = KP_GETCY(KP_Y_VAL_C,KP_HEIGHT_VAL_C,4,16);
            w = KP_GETW(KP_WIDTH_VAL_C,(4/3));
            h = KP_GETH(KP_HEIGHT_VAL_C,(24/5));
            onLBSelChanged = "ctrlSetText [758039, groupId (KPLIB_groups select (lbCurSel 758038))]";
        };

        class KP_ButtonJoin: KPLIB_GUI_PRE_InlineButton {
            text = "$STR_KPLIB_JOIN";
            x = KP_GETCX(KP_X_VAL_C,KP_WIDTH_VAL_C,3,4);
            y = KP_GETCY(KP_Y_VAL_C,KP_HEIGHT_VAL_C,4,16);
            w = KP_GETW(KP_WIDTH_VAL_C,4);
            h = KP_GETH(KP_HEIGHT_VAL_C,(48/5));
            onButtonClick = "[] call KPLIB_fnc_join";
        };

        class KP_ButtonCreate: KP_ButtonJoin {
            text = "$STR_KPLIB_CREATE";
            y = KP_GETCY(KP_Y_VAL_C,KP_HEIGHT_VAL_C,17,48);
            onButtonClick = "[] call KPLIB_fnc_create";
        };

        class KP_RenameEdit: KPLIB_GUI_PRE_EditBox {
            idc = 758039;
            x = KP_GETCX(KP_X_VAL_C,KP_WIDTH_VAL_C,0,1);
        ...
        class KP_ButtonRename: KP_ButtonJoin {
            text = "$STR_KPLIB_RENAME";
            y = KP_GETCY(KP_Y_VAL_C,KP_HEIGHT_VAL_C,11,24);
            h = KP_GETH(KP_HEIGHT_VAL_C,24);
            onButtonClick = "[] call KPLIB_fnc_rename";
        };

        class KP_BoxViewFoot: KPLIB_GUI_PRE_EditBox {
            idc = 7580310;
            style = 65;
        ...
        class KP_ComboTerrain: KPLIB_GUI_PRE_Combo {
            idc = 7580313;
            x = KP_GETCX(KP_X_VAL_C,KP_WIDTH_VAL_C,1,2);
            y = KP_GETCY(KP_Y_VAL_C,KP_HEIGHT_VAL_C,12,16);
            w = KP_GETW(KP_WIDTH_VAL_C,2);
        ...
        class KP_SliderSound: KPLIB_GUI_PRE_Slider {
            idc = 7580317;
            x = KP_GETCX(KP_X_VAL_C,KP_WIDTH_VAL_C,0,1);
            y = KP_GETCY(KP_Y_VAL_C,KP_HEIGHT_VAL_C,23,24);
            w = KP_GETW(KP_WIDTH_VAL_C,1);
            h = KP_GETH(KP_HEIGHT_VAL_C,24);
            tooltip = "$STR_KPLIB_TTSOUND";
            onSliderPosChanged = "ctrlSetText [7580316, format ['%1%2', round (_this select 1), '%']]";
        };

        class KP_ApplyButton: KPLIB_GUI_PRE_DialogButtonC {
            text = "$STR_KPLIB_APPLY";
            onButtonClick = "[] call KPLIB_fnc_save";
        };

        class KP_DialogCross: KPLIB_GUI_PRE_DialogCrossC {};
    };
};
