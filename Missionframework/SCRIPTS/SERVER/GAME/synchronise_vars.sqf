...
while {true} do {
    waitUntil {sleep 0.25;
        !(_KP_liberation_fob_resources_old isEqualTo KPLIB_fob_resources)
        || _KP_liberation_supplies_global_old != KPLIB_supplies_global
        || _KP_liberation_ammo_global_old != KPLIB_ammo_global
        || _KP_liberation_fuel_global_old != KPLIB_fuel_global
        || _unitcap_old != unitcap
        || _KP_liberation_heli_count_old != KPLIB_heli_count
        || _KP_liberation_plane_count_old != KPLIB_plane_count
        || _KP_liberation_heli_slots_old != KPLIB_heli_slots
        || _KP_liberation_plane_slots_old != KPLIB_plane_slots
        || _combat_readiness_old != combat_readiness
        || _resources_intel_old != resources_intel
        || _infantry_cap_old != infantry_cap
        || _KP_liberation_civ_rep_old != KPLIB_civ_rep
        || _KP_liberation_guerilla_strength_old != KPLIB_guerilla_strength
    ...
    _KP_liberation_fob_resources_old = +KPLIB_fob_resources;
    _KP_liberation_supplies_global_old = KPLIB_supplies_global;
    _KP_liberation_ammo_global_old = KPLIB_ammo_global;
    _KP_liberation_fuel_global_old = KPLIB_fuel_global;
    _unitcap_old = unitcap;
