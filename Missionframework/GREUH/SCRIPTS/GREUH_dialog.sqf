...
_dialog = createDialog "KPLIB_Menu";
waitUntil { dialog };

if ( KPLIB_allow_customsquads ) then {
	ctrlShow [501, false];
} else {
	ctrlShow [501, true];
...
if ( KPLIB_allow_platoonview ) then {
	ctrlShow [601, false];
} else {
	ctrlShow [601, true];
...
if ( KPLIB_allow_viewdistance ) then {
	ctrlShow [701, false];
...
if ( KPLIB_allow_worldquality ) then {
	ctrlShow [801, false];
} else {
	ctrlShow [801, true];
...
if ( KPLIB_allow_mapmarkers ) then {
	ctrlShow [901, false];
} else {
	ctrlShow [901, true];
...
while { dialog && alive player } do {
	...
	if ( KPLIB_allow_platoonview ) then { ctrlShow [612, show_platoon]; };
	if ( KPLIB_allow_mapmarkers ) then { ctrlShow [912, show_teammates]; };
...
profileNamespace setVariable [ "KPLIB_OPTIONS_PROFILE",greuh_options_profile ];
