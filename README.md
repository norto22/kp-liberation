...
For the rope test, choose **Armed / Ghost Hawk**. It receives FRIES and four 36 m ACE ropes at spawn. On reaching the LZ approach area, the system prepares FRIES, then waits for a low, slow hover before ropes deploy and passengers descend automatically, one at a time. Passenger groups are preserved. The default Light / Hummingbird lacks ACE rope support and uses a landing instead. ACE's [fast-roping framework](https://ace3.acemod.org/wiki/framework/fastroping-framework) describes aircraft compatibility. Server RPT entries tagged `[TAXI]` report approach failure, missing FRIES hardware, preparation stage, hover readiness, and deployed-rope count.
...
- For a longer insertion flight, verify the pilot slows before the LZ and continues correcting position and altitude after FRIES reaches stage 2. It must not stop permanently outside the hover bounds; `[TAXI]` stabilization logs show any remaining distance, height, or speed problem.
- Verify that a settled hover above 26 m can deploy when the supplied ropes reach the terrain below both attachment points. Readiness uses actual hook-to-ground distance and the configured rope length with 0.5 m slack, not a fixed altitude cutoff. The speed limit remains 3 m/s; the RPT reports rope reach separately from speed. A manually deployed shorter rope must not allow a passenger to be sent down beyond its reach.
...
The readiness and rope-cleanup decisions have executable SQF-VM regressions, including the reported 34 m hover. See [the test instructions](tools/README.md#taxi-sqf-regression-tests). These exercise the production SQF decisions; they do not simulate Arma's helicopter AI, ACE rope physics, or network replication. Those still require the in-game checks above.
