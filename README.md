...
The taxi starts airborne about 2.5 km away and flies to clear ground within 300 m of the departure FOB. The search checks aircraft-sized empty positions across that area. Landing-gear terrain must be dry and reasonably level; geometry checks across a 16 m radius (larger for larger loaded airframes) reject roofs, overhangs, walls and obstructing vehicles without excluding an entire field because of nearby objects' bounding spheres. The site is rechecked during descent; another taxi's reserved footprint is excluded. Keep the immediate touchdown area clear until it lands. If no suitable pickup site exists, the request is rejected before fuel is charged. Flight routing still relies on Arma's AI; this is landing-site validation, not a guarantee against every AI collision.
